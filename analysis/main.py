...
async def main():
    ...
    async with await psycopg.AsyncConnection.connect(host="localhost", dbname="postgres", user="rohits") as aconn:
        ...
        all_sortings, all_imdb_score, international_sortings, international_imdb_score = await asyncio.gather(get_all_sortings(acur1), get_all_imdb_score_data(acur2), get_international_sortings(acur3), get_international_imdb_score_data(acur4))
