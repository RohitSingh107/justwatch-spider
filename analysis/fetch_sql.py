...
async def get_international_sortings(cur):
    ...
    f = await cur.execute(q)

    international_unordered = await f.fetchall()
    all_sortings = await get_all_sortings(cur)


    # international_unordered, all_sortings = await asyncio.gather(f.fetchall(), get_all_sortings(cur))
